...
<script type="text/javascript">
    window.onload=function(){
        if (getchange()==1){
        $("#repository").addClass('leftmenu-title-down');
    }};
    ...
    function beforeRemove(treeId, treeNode) {
        return  layer.msg("确认删除 节点 -- " + treeNode.name + " 吗？", {icon: 1});
    }
    ...
    function onRename(e, treeId, treeNode) {/*编辑增加*/
        ...
        if (treeNode.id) {
            ajaxUrl = ajaxUrl + "/dictSubject/update.do";
        } else {
            ajaxUrl = ajaxUrl + "/dictSubject/add.do";
        }
        $.ajax({
            type: "post",
            url: ajaxUrl,
    ...
    function remove() {
        ...
        layer.confirm("您确定删除此节点", {
            ...
            var ajaxUrl = basecontextPath + "/dictSubject/delete.do";
            $.ajax({
                type: "post",
                url: ajaxUrl,
                datatype: "json",
    ...
    $(document).ready(function () {
        var url = basecontextPath + "/dictSubject/getAllChildSubject.do";
