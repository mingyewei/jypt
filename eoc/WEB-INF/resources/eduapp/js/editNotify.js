...
function getZtree(id) {
	...
	$.getJSON(basecontextPath+"/NotifyController/zTreeSelectUser.do?id="+id, function(data) {
		if (data.code == 1) {
			var newTreeNode = [];
			for (var i = 0; i < data.data.orgs.length; i++) {
				var treeNodeData = {};
				...
				for (var f = 0; f < newTreeNode.length; f++) {
					if (typeof(newTreeNode[f].id) == "string") {
						for (var h = 0; h < data.data.selectedUsers.length; h++) {
							if ((newTreeNode[f].id).replace(/users/, "") == data.data.selectedUsers[h].id) {
...
//附件删除filetips1
$("#filetips1").on("click","#delAttachmentFile",function(){
	var attachmentId=$("#attachmentId").val();
	$.ajax({
		type: "post",
		url:basecontextPath+"/NotifyController/delNotifyFile.do",
